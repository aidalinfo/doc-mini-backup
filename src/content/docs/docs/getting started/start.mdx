...
Nous avons bien récupéré notre base de données !

### Cli 

Il est aussi possible de restaurer une sauvegarde depuis la ligne de commande.

```bash
docker exec mini-backup /app/backup-cli restore mongo last
```

Cette commande nous permet de lancer la cli avec l'argument `restore` et de spécifier le nom du backup à restaurer ainsi que la version à restaurer.

Il est aussi possible de demander un backup antérieur.
...
Des tutoriels et configuration d'exemple pour chaque type de backup sont dans la roadmap et nous espérons vous les proposer très bientôt.